...
function decide(index) 
    ...
    c == 4 && (println("Too much symbols excluded"); return "")
...
function swapper(nums::Matrix{Int8})
    ...
    if indices[1] <= 4
        # swap the nums through
        nums[indices[1]], nums[indices[2]] = nums[indices[2]], nums[indices[1]]
    else
        indices[1] = 0
        indices[2] += 1
    end
...
function procedure()   
    ...
    println("Excluded symbols: b = bracket | + = + | - = - | * = * | / = / |  Example: b+-")
    excludeStr = readline()
    ...
    clear()
    while true
        # copy to keep the original orde
        ...
        # one-line multi nested loop to iterate over all possible symbol combinations
        for ac in 0:3, bc in 0:3, cc in 0:3, lb in 0:2, rb in 1:3     
            lb == 2 && (rb += 1)    # lb == left bracket
            rb > 3 && (rb = 3)      # rb == right bracket 

            decideA, decideB, decideC = decide(ac), decide(bc), decide(cc)
            if decideA == "" || decideB == "" || decideC == ""
                sleep(3)
                return
            end
            # create the expression
            # decide returns the corresponding mathmatical symbol
            # decide gets compared to "" to return if too many symbols were excluded (see ret val options of decide)
            # converting the nums integers to floats for precise comparesion
            expr = string(bracket(lb, 0, true), convert(Float64, nums[1]), " ", 
            decideA, " ", bracket(lb, 1, true), convert(Float64, nums[2]), bracket(rb, 1, false), " ", decideB, " ",  
            bracket(lb, 2, true), convert(Float64, nums[3]), bracket(rb, 2, false), " ", decideC, " ", convert(Float64, nums[4]), bracket(rb, 3, false))
